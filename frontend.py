'''
A program  storing books information: title, Author, Year, ISBN
User can:
Veiw all records, Search an Entry,Add an Entry, Update an Entry, Delete, close
'''

from tkinter import *

# Not to mix codes for GUI and database, we write another Script for database and import it here
...
class Data(object):
    def __init__(self, window):
        ...
        self.window.wm_title("USMAN BookStore")
    ...
    def search_command(self):
        self.list1.delete(0, END)
        for item in database.search(self.title_text.get(), self.author_text.get(), self.year_text.get(),
                                    self.isbn_text.get()):
    ...
    def add_command(self):
        ...
        self.list1.insert(END, "Title: ", self.title_text.get(), "Author: ", self.author_text.get(), "Year: ",
                          self.year_text.get(), "ISBN: ", self.isbn_text.get(), "Added Successfully!")

    def update_command(self):
        database.update(NULL, self.title_text.get(), self.author_text.get(), self.year_text.get(), self.isbn_text.get())
    ...
    def get_selected_row(self, event):
        ...
        if len(self.list1.curselection()) > 0:
            ...
            self.list1.insert(END, "No List to Select From!!")
